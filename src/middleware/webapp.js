...
function jsx(node) {
  ...
  let type = node[0], props, children;
...
const css = styles => {
  ...
  return Object.keys(styles || {}).reduce((rules, selector) => {
...
const sha1 = string =>
  crypto.createHash('sha256').update(string).digest('base64')
    .substr(0, 8).replace(/\+/g, '-').replace(/\//g, '_');
...
module.exports = config => {
  ...
  const {components} = site(
    Object.entries(views).reduce((views, [name, view]) => ({
      ...views,
      [name]: view.component
    }), {}),
    config,
  ...
  return (req, res, next) => {
    ...
    const asset = req.path.match(/^\/(css)\/([^\.]+)\.([^\.]+)\.css$/) ||
    ...
    if (asset) {
      ...
      res.render = (view, {meta, ...props}) => {
        ...
        ids.forEach(function add(name) {
          ...
          assets[name] = {
            styles: (styles ? ['/css/' + name + '.' + sha1(css(styles)) + '.css'] : [])
              .concat(includes.styles || []),
            scripts: ['/js/' + name + '.' + sha1(script(name, component)) + '.js']
              .concat(includes.scripts || [])
          };
          dependencies.forEach(add);
        });
        res.end('<!doctype html>' +
          reactDOM.renderToStaticMarkup(jsx(
            [components.site, {
              ...meta,
              view: ids[1],
              props,
              styles: Array.from(new Set(Object.values(assets).reduce((urls, {styles}) => urls.concat(styles), []))),
              scripts: Array.from(new Set(Object.values(assets).reduce((urls, {scripts}) => urls.concat(scripts), []))),
            }]
          )));
      };
